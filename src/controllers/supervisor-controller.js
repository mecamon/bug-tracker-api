...
async function deleteUser(httpRequest) {
  ...
  const deletedFromCompanyInfo = await repo.companyRepo.pull(idCompany, {
...
async function getResumeOfUser(httpRequest) {
  ...
  const userResume = calculateResume(user, reports);
...
//Calculates the user resume--
function calculateResume(user = {}, reports = []) {
  const userResume = {
  ...
  const percentageCancelled = parseInt(
    (reportsCancelled * 100) / reports.length
  );
...
//Checks if this is a valid modified object--
function validate({
